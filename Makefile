NAME	= libft.a

SRCS	= ft_isdigit.c ft_memset.c ft_strjoin.c ft_strtrim.c ft_isprint.c \
ft_putchar_fd.c ft_strlcat.c ft_substr.c ft_atoi.c ft_itoa.c \
ft_strlcpy.c ft_tolower.c ft_bzero.c ft_strlen.c ft_putendl_fd.c ft_putnbr_fd.c ft_putstr_fd.c \
ft_toupper.c ft_calloc.c ft_memchr.c  ft_strmapi.c ft_isalnum.c \
ft_memcmp.c ft_split.c ft_strncmp.c ft_isalpha.c ft_memcpy.c ft_strchr.c\
ft_strnstr.c ft_isascii.c ft_memmove.c ft_strdup.c ft_strrchr.c ft_striteri.c \
ft_lstnew.c			\
ft_lstadd_front.c	\
ft_lstsize.c		\
ft_lstlast.c		\
ft_lstadd_back.c	\
ft_lstclear.c		\
ft_lstdelone.c		\
ft_lstiter.c		\
ft_lstmap.c			\

CC	= gcc
CFLAGS	= -Wall -Wextra -Werror
LIBC	= ar rcs
RM	= rm -f
OBJS	= $(SRCS:.c=.o)

.c.o:
	$(CC) $(CFLAGS) -c $< -o $(<:.c=.o)

$(NAME): $(OBJS)
	$(LIBC) $(NAME) $(OBJS) libft.h

all:$(NAME)

clean:
	$(RM) $(OBJS)

fclean:
	$(RM) $(NAME)

re:fclean all

.PHONY:all,clean,fclean,re